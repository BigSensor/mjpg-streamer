...
static struct fb_param gfb_param;
...
int input_init(input_parameter *param, int id)
{
    ...
    while(1) {
        ...
        case 9:
	        ...
	        case 4:
	        	...
	        	break;
	        default:
	        	DBG("default case, unsupported yuv format\n");
            	help();
            	return 1;
            }
            break;
    ...
    IPRINT("Format............: %s\n", (format == V4L2_PIX_FMT_MJPEG) ? "MJPEG" : "YUV");
    if (gfb_param.fb_dev >= 0) {
        IPRINT("Display framebuffer: /dev/fb%d\n", gfb_param.fb_dev);
    } else {
        IPRINT("Display framebuffer: no\n");
    }
    ...
    if (gfb_param.fb_dev >= 0)
	    if (fb_open(gfb_param.fb_dev)) {
		IPRINT("/dev/video%d open failed\n", gfb_param.fb_dev);		
		exit(EXIT_FAILURE);
	}
    return 0;
}
...
int input_stop(int id)
{
    ...
    if (gfb_param.fb_addr != NULL && gfb_param.fb_dev >= 0)
        memset(gfb_param.fb_addr, 0, gfb_param.fb_size);
...
void help(void)
{
    ...
    " [-y | --yuv ]..........: enable YUV format(1:yuyv, 2:yvyu, 3:uyvy 4:vyuy) and disable MJPEG mode\n" \
...
void *cam_thread(void *arg)
{
    ...
    // caculate fps
    int yframe_num;
    struct timeval ycur_time, ylast_time;
    int fps_time;
    int need_compress = 1;                  // 0: don't compress; 1: compress
    ...
    while(!pglobal->stop) {
        ...
        if(pcontext->videoIn->formatIn != V4L2_PIX_FMT_MJPEG) {
            if (1 == need_compress) {
                ...
                pglobal->in[pcontext->id].size = compress_yuv_to_jpeg(pcontext->videoIn, pglobal->in[pcontext->id].buf, pcontext->videoIn->framesizeIn, gquality, pcontext->videoIn->formatIn, &gfb_param);
            }
            yframe_num++;
            if (1 == yframe_num) {
                gettimeofday(&ylast_time, NULL);
            }

            if((100 == yframe_num))
            {
                yframe_num = 0;
                gettimeofday(&ycur_time, NULL);
                fps_time = ((ycur_time.tv_sec * 1000000 + ycur_time.tv_usec) - (ylast_time.tv_sec * 1000000 + ylast_time.tv_usec)) / 1000;                    
                IPRINT("Capture%s 100 frame yuv cost %d ms, fps=%.1f\n", need_compress?" and compress":"", fps_time, (100*1000/(double)(fps_time)));
            }
        } else {
...
int fb_open(int dev) 
{
	...
	if (dev < 0)
		return -1;
	sprintf(devname, "/dev/fb%d", dev);
	if ((fb = open(devname, O_RDWR)) < 0) {
		IPRINT("Open framebuffer '%s' failed: %d, %s\n", devname, errno, strerror(errno));
		return -1;
	}

	memset(&var, 0, sizeof(struct fb_var_screeninfo));
	if (ioctl(fb, FBIOGET_VSCREENINFO, &var)) {
		IPRINT("Get var screeninfo failed, %d\n", errno);
		return -1;
	}

	/* initialize global vars */
	gfb_param.fb_xres	= var.xres;
	gfb_param.fb_yres	= var.yres;
	gfb_param.fb_bpp	= var.bits_per_pixel;
	DBG("Framebuffer setting:\n");
	DBG("  X-res, Y-res ........ %4d,  %4d\n", var.xres, var.yres);
	DBG("  bpp ................. %4d\n", var.bits_per_pixel);
	DBG("  pixclock ............ %4d\n", var.pixclock);
	DBG("  left,  right ........ %4d,  %4d\n", var.left_margin, var.right_margin);
	DBG("  upper, lower ........ %4d,  %4d\n", var.upper_margin, var.lower_margin);
	DBG("  hsync, vsync ........ %4d,  %4d\n", var.hsync_len, var.vsync_len);
	DBG("\n");

	memset( &fix, 0, sizeof(struct fb_fix_screeninfo) );
	if (ioctl(fb, FBIOGET_FSCREENINFO, &fix)) {
		IPRINT("Get fix screeninfo failed, %d\n", errno);
		return -1;
	}

	gfb_param.fb_size = fix.smem_len;
	gfb_param.fb_addr= (unsigned char *)mmap(0, gfb_param.fb_size, PROT_READ | (1 * PROT_WRITE), MAP_SHARED, fb, 0);
	if (gfb_param.fb_addr == MAP_FAILED) {
		IPRINT("Cannot map Frame Buffer, fbsize is %u, errno = %d\n", gfb_param.fb_size, errno);
		return -1;
	}
	return 0;
}
